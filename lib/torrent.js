...
class Torrent extends EventEmitter {
  ...
  _onMetadata (metadata) {
    ...
    if (this.skipVerify) {
      ...
      if (this._fileModtimes && this._store === FSChunkStore) {
        ...
        if (this.so) this._markFilesVerified(this.so)
        this._verifyPieces(onPiecesVerified)
      }
    }
  }
   ...
   *  excludedFiles contained string with file indexes to exclude.
   */
  _markFilesVerified (excludedFiles) {
    excludedFiles = excludedFiles.split(',').map((str) => parseInt(str.trim()))
    this.files.forEach((file, index) => {
      if (!excludedFiles.includes(index)) {
        const start = excludedFiles.includes(index - 1) ? file._startPiece + 1 : file._startPiece
        const end = excludedFiles.includes(index + 1) ? file._endPiece - 1 : file._endPiece
        this._markVerifiedGroup(start, end)
      }
    })
  }
