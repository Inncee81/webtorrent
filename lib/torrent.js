...
const STREAM_LENGTH_PERMISSION = 300000000 // 300 MB
...
class Torrent extends EventEmitter {
  ...
  _asyncVerifyPieces (state) {
    if (this.strategy === 'sequential' && state.verified.length > 2 && !state.verified.slice(-2).includes(true)) {
      return state.cb()
    }
    ...
    setTimeout((fn) => {
      ...
      if (state.counter < state.verifiers.length) {
        state.verifiers[state.counter]()
        fn()
      } else if (state.counter === state.verifiers.length) {
        state.cb()
      }
      state.counter++
    }, 100, this._asyncVerifyPieces.bind(this, state))
  }

  _verifyPiece (index, state) {
    return () => {
      if (this.destroyed || !this.pieces[index]) return

      const downloaded = this.pieceLength * state.verified.filter(valid => valid).length
      if (downloaded >= STREAM_LENGTH_PERMISSION) {
        this.emit('streamble')
        this.streamble = true
      }
