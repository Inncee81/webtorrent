...
const STREAMBLE_EDGE = 300000000 // 300 MB
const VERIFICATION_TIMEOUT = 200 // ms
const VERIFICATION_CHUNK = 150000000 // 150 mb
...
class Torrent extends EventEmitter {
  ...
  constructor (torrentId, client, opts) {
    ...
    this._store = opts.store && opts.store.engine ? opts.store.engine : FSChunkStore
    this._storeSecondaryName = opts.store && opts.store.secondaryName ? opts.store.secondaryName : noop
    this._storeDriver = opts.store && opts.store.driver ? opts.store.driver : null
    this._getAnnounceOpts = opts.getAnnounceOpts
    this._so = null
    this._soPieceMapper = new Map()

    if (!isNaN(opts.so)) {
      this._so = [Number.parseInt(opts.so)]
    } else if (opts.so && typeof opts.so.valueOf() === 'string') {
      this._so = parseRange(this._so)
    }
  ...
  get downloaded () {
    ...
    const len = this.pieces.length - 1
    const pieces = this._requestedPieces()

    return pieces.map(index => {
      if (this.bitfield.get(index)) { // verified data
        return (index === len) ? this.lastPieceLength : this.pieceLength
      } else { // "in progress" data
        const piece = this.pieces[index]
        return piece ? piece.length - piece.missing : 0
      }
    }).reduce((partialSum, len) => partialSum + len, 0)
  }
  ...
  get progress () {
    ...
    if (this._so && this.length) {
      const pieces = this._requestedPieces()
      length = pieces.map(index => {
        return (index === lastPieceIndex) ? this.lastPieceLength : this.pieceLength
      }).reduce((partialSum, len) => partialSum + len, 0)
    } else if (this.length) {
      length = this.length
    }

    return length ? this.downloaded / length : 0
  }
  ...
  _onMetadata (metadata) {
    ...
    this.store = new ImmediateChunkStore(
      new this._store(this.pieceLength, {
        ...
        length: this.length,
        name: this.infoHash,
        secondaryName: this._storeSecondaryName(this)
      })
    )
    ...
    if (this._so) {
      this.files.forEach((v, i) => {
        if (this._so.includes(i)) this.files[i].select(true)
      })
    } else {
    ...
    if (this.skipVerify) {
      ...
      if (this._fileModtimes && this._store === FSChunkStore) {
        ...
        if (this._so) this._markFilesVerified()
        this._verifyPieces(onPiecesVerified)
      }
    }
  }
  ...
  _verifyPieces (cb) {
    ...
    const requestedPieces = this._requestedPieces()

    for (let index, i = 0; requestedPieces.length; i++) {
      index = (i % 2) ? requestedPieces.pop() : requestedPieces.shift()
      state.verifiers.push(this._verifyPiece(index, state))
    }

    this._asyncVerifyPieces(state)
  }

  _cachedPiecesOver (state) {
    return this.strategy === 'sequential' && state.verified.length > 2 &&
      !(state.verified.length % 2) && !state.verified.slice(-2).includes(true)
  }

  _asyncVerifyPieces (state) {
    setTimeout(async (fn) => {
      if (this.destroyed) return

      let accumulator = 0

      while (state.counter < state.verifiers.length && accumulator < VERIFICATION_CHUNK) {
        await state.verifiers[state.counter]()
        if (this._cachedPiecesOver(state)) return state.cb()
        accumulator += this.pieceLength
        state.counter++
      }

      this._gcVerification()

      if (state.counter >= state.verifiers.length) state.cb()
      else fn()
    }, VERIFICATION_TIMEOUT, this._asyncVerifyPieces.bind(this, state))
  }

  _verifyPiece (index, state) {
    return () => new Promise(resolve => {
      this._debug('verify piece index %s', index)
      if (this.destroyed || !this.pieces[index]) return resolve()

      const downloaded = this.pieceLength * state.verified.filter(valid => valid).length
      if (downloaded >= STREAMBLE_EDGE && !this.streamble) {
        this.streamble = true
        this.emit('streamble')
      }
      ...
      this.store.get(index, (err, buf) => {
        ...
        sha1(buf, hash => {
          ...
          if (hash === this._hashes[index]) {
            if (!this.pieces[index]) return resolve()
            this._debug('piece verified %s', index)
            state.verified.push(true)
  ...
  /**
   *  Mark as verified unneeded pieces
   */
  _markFilesVerified () {
    const requestedPieces = this._requestedPieces()

    this.pieces.forEach((piece, index) => {
      if (!requestedPieces.includes(index)) this._markVerified(index)
    })
  }
  ...
  _requestedPieces () {
    ...
    const fn = () => {
      if (!this._so) return [...this.pieces.keys()]

      const requestedRanges = this._so.map(index => {
        return this._range(this.files[index]._startPiece, this.files[index]._endPiece)
      })
      const pieces = [].concat.apply([], requestedRanges)
      return [...new Set(pieces)]
    }

    if (!this._soPieceMapper.has(this._so)) {
      this._soPieceMapper.set(this._so, fn())
    }

    return [...this._soPieceMapper.get(this._so)]
  }
  ...
  select (start, end, priority, notify, fileStream = null) {
    if (this.destroyed) throw new Error('torrent is destroyed')

    if (start < 0 || end < start || this.pieces.length <= end) {
      throw new RangeError(`invalid selection ${start} : ${end}`)
    }
  ...
  deselect (start, end, priority, fileStream = null) {
    if (this.destroyed) throw new Error('torrent is destroyed')
    ...
    for (let i = 0; i < this._selections.length; ++i) {
      ...
      if (s.from === start && s.to === end && s.priority === priority && s.fileStream === fileStream) {
  ...
  _checkDone () {
    ...
    this.files.forEach((file, index) => {
      if (file.done) return
      if (this._so && !this._so.includes(index)) return
    ...
    const done = this._requestedPieces().every(index => this.bitfield.get(index))
