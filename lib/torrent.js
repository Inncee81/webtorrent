...
class Torrent extends EventEmitter {
  ...
  _verifyPieces (cb) {
    const state = { counter: 0, fns: [], cb: cb }
    ...
    for (let index, i = 0; requestedPieces.length; i++) {
      ...
      state.fns.push(this._verifyPiece(index))
    }
  ...
  _asyncVerifyPieces (state) {
    setTimeout((fn) => {
      ...
      if (state.counter < state.fns.length) {
        state.fns[state.counter]()
        fn()
      } else if (state.counter === state.fns.length) {
        state.cb()
      }
      state.counter++
