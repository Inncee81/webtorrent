...
class File extends EventEmitter {
  ...
  createReadStream (opts) {
    if (!this._torrent || this._torrent._destroyed) throw new Error('Torrent removed')
    if (this.length === 0) {
    ...
    this._torrent.select(fileStream._startPiece, fileStream._endPiece, true, () => {
      if (!fileStream.destroyed) fileStream._notify()
    }, fileStream)
    eos(fileStream, () => {
      debug('Suppress file stream errors')
      if (this._destroyed) return
      if (!this._torrent.destroyed) {
        this._torrent.deselect(fileStream._startPiece, fileStream._endPiece, true, fileStream)
      }
    })
