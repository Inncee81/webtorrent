 ...
 */
exports.createTCPIncomingPeer = (conn, client) => {
...
class Peer {
  ...
  onConnect () {
    ...
    if (this.type === 'tcpOutgoing' && !!this.swarm) {
      conn.pipe(this.swarm.client.throttleGroups.down.throttle())
        .pipe(wire)
        .pipe(conn)
    } else if (this.type === 'tcpIncoming' && !!this.client) {
      conn.pipe(wire)
        .pipe(this.client.throttleGroups.up.throttle())
        .pipe(conn)
    } else {
